...
def run_single_simulation(config, current_topic_or_task, topic_index):
    ...
    for i, p in enumerate(participants):
        ...
        if pid is None:
            pid = f'unk_participant_{i}'; logging.warning(f"Participant missing ID, using {pid}"); p['persona_id'] = pid
        if pid not in participant_map:
            participant_ids.append(pid); participant_map[pid] = p
        else:
            logging.warning(f"Duplicate ID '{pid}'")
    if not participant_ids:
        logging.error("No valid participant IDs found."); return False
                        ...
                        if not zero_turn_override:
                            ...
                            if parsed_choice_id and parsed_choice_id == last_speaker_id: logging.warning(f"Mod chose last speaker. Overriding."); moderator_choice_method='fallback_override'; parsed_choice_id=None
                                ...
                                if zero_turn_fallback_candidates: chosen_participant_id = random.choice(zero_turn_fallback_candidates); logging.info(f"Fallback prioritizing zero turns: '{chosen_participant_id}'"); moderator_choice_method += '_zero_priority'
                                    ...
                                    for pid in eligible_fallback_ids:
                                        weight = participation_weights.get(pid, initial_weight)
                                        if weight > max_weight:
                                            max_weight = weight; candidates = [pid]
                                        elif weight == max_weight:
                                            candidates.append(pid)
                                    if candidates:
                                        chosen_participant_id = random.choice(candidates)
                                        logging.info(f"Fallback choice (Highest Weight): '{chosen_participant_id}'")
                                    else:
                                        logging.error("CRITICAL FALLBACK FAILURE. Aborting.")
